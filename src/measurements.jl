...
function spot_defects(thetas, lattice_type="square")
    ...
    qs_matrix = CUDA.zeros(Int, Lx, Ly, R) # charges
    #= 
    One could define  
    ...
    if lattice_type == "square"
        @cuda threads = block3D blocks = grid3D kernel_spot_defects_square!(mod.(thetas, 2Float32(pi)), qs_matrix, Lx, Ly, R)
    elseif lattice_type == "triangular"
        @cuda threads = block3D blocks = grid3D kernel_spot_defects_triangular!(mod.(thetas, 2Float32(pi)), qs_matrix, Lx, Ly, R)
    else
        error("lattice_type should be one of : square, triangular")
    end

    qs_vector_tmp = reshape(qs_matrix, Lx * Ly, R)
    ...
    for r in 1:R
        ...
        # println("qqq ", sum(qs_vector_tmp[:, r]))
        # println("qqq ", indices_defects)
        qs[r] = qs_vector_tmp[indices_defects, r]
        xs[r] = xs_vector_tmp[indices_defects, r]
        ys[r] = ys_vector_tmp[indices_defects, r]
    end

    number_defects_plus = reduce(+, qs_vector_tmp .== 1, dims=1)[1, :]
    number_defects_minus = reduce(+, qs_vector_tmp .== -1, dims=1)[1, :]
...
function kernel_spot_defects_square!(thetas, qs_matrix, Lx::Int, Ly::Int, R::Int)
